...
var defaults = {
  ...
  imgPNG: false,                  // generate PNG images using batik-rasterizor
  removeJax: true,                // remove MathJax <script> tags?
  speakText: false,               // add spoken annotations to svg output?
  svgPreview: false,              // turn SVG into a MathJax preview, and keep the jax
...
//
//  Convert the SVG to a PNG image, using the batik-rasterizer
//  and remove previous SVG data url.
//  Need `data.dpi` option.
//
function GetPNG() {
    var n = 0,
        nodes = document.getElementsByClassName("MathJax_SVG"),
        defs = document.getElementById("MathJax_SVG_glyphs") || document.getElementsByTagName('defs')[0];
    if (!defs) {
        return console.log('no mathJax object is found in html');
    }
    for (var i = nodes.length-1; i >= 0; i--) {
        var svg = nodes[i].getElementsByTagName("svg")[0];
        var css = svg.style.cssText +
        ...
        var options = { flag: 'w' };
        var tmpfile = path.join(__dirname, 'tempFile');
        fs.writeFileSync(tmpfile+".svg", svgfile, options);
        if (!data.dpt) data.dpi = 300;
        var result = execSync.exec("java -jar "+ path.join(__dirname, "../batik/batik-rasterizer.jar") + " -dpi "+data.dpi+" "+tmpfile+".svg");
        if (true) {
            var buffer = fs.readFileSync(tmpfile+".png");
            var img = MathJax.HTML.Element("img",{
                src:"data:image/png;base64,"+buffer.toString('base64'), style:{cssText:css},
                className: "MathJax_PNG_IMG"
            });
...
//
//  Move the SVG output to a MathJax_Preview node preceeding the
//  nodes script element, and remove any previous preview.
...
function GetPreviews() {
  ...
  for (var i = jax.length-1; i >= 0; i--) {
    ...
    var preview = MathJax.HTML.Element(frame.nodeName,{className:"MathJax_Preview"});
...
function TypesetDone(result,selector) {
  ...
  if (data.renderer === "SVG") {
    ...
    //
    if (data.useGlobalCache && !data.imgSVG && !data.imgPNG) {
    ...
    //
    if (data.imgPNG && !data.imgSVG) {GetPNG()}
    //
    //  Create <img> tags for SVG output
    //
    if (data.imgSVG && !data.imgPNG) {GetIMG()}
    //
    //  Make SVG into previews
    //
    if (data.svgPreview) {GetPreviews()}
  }
  //
  ...
  //
  if (data.removeJax && selector && !data.svgPreview) {
...
function StartQueue() {
  ...
  SVG.config.useGlobalCache = data.useGlobalCache && !data.imgSVG && !data.imgPNG;
