...
var userCallback;     // Reference to the user's callback. 
...
function ReportError(message,currentCallback) {
  AddError(message);
  (currentCallback||callback||userCallback)({errors: errors});
}
...
function AddError(prefix,message,nopush) {
  var n = MathJax != null ? MathJax.ElementJax.ID + 1 : 1;
  if (prefix !== "") {prefix += " ("+n+"): "}
...
function StartQueue() {
  ...
  data = item[0]; callback = item[1]; userCallback = callback;
  inputDocument = jsdom(data.html);
  content.innerHTML = inputDocument.body.innerHTML; //data.html;  //  %%% disable <script> tags?
  ...
  if (!errors.length) {
    ...
    if (data.renderer === "SVG" || data.renderer === "IMG" || data.renderer === "PNG") {
      ...
      for (i = nodes.length-1; i>=0; i--) {
        ...
        nodes[i].setAttribute("id", jax.inputID);
      }
      //
      //  Fix incorrect roles
      //
        ...
        fs.writeFile(tmpSVG,svg,function (err) {
          ...
          exec(batikCommand, function (err,stdout,stderr) {
            ...
            fs.readFile(tmpPNG,null,function (err,buffer) {
              if (!check(err)) {
                var pngJax = MathJax.Hub.getJaxFor(nodes[i]);
                var img = MathJax.HTML.Element("img",{
                ...
                SetWH(img,w,h); // work around jsdom bug with width and height CSS
                if (data.speakText) img.setAttribute("alt",pngJax.speech);
                nodes[i].parentNode.replaceChild(img,nodes[i]);
              }
