...
var defaults = {
  ...
  htmlElement: false,             // DOM element for HTML output?
  css: false,                     // return CSS for HTML output?
  mml: false,                     // return mml output?
  mmlElement: false,              // DOM element for MML output?
  svg: false,                     // return svg output?
  svgElement: false,              // DOM element for SVG output?
...
function GetMML(result) {
  ...
  try {
    ...
    if(data.mmlElement) result.mmlElement = jsdom(result.mml).body.firstChild;
  } catch(err) {
...
function GetHTML(result) {
  ...
  if (data.htmlElement) result.htmlElement = html;
...
function GetSVG(result) {
  ...
  //
  result.svg = svgdata;
  if (data.svgElement) result.svgElement = svg;
