...
var defaults = {
  ...
  phtml: false,                    // return pHTML output?
  phtmlNode: false,             // DOM element for pHTML output?
  css: false,                     // return CSS for HTML output?
  pcss: false,                     // return CSS for pHTML output?
...
var PHTMLSTYLES = "";    // filled in when PreviewHTML is loaded
...
function ConfigureMathJax() {
  window.MathJax = {
    ...
    //
    jax: ["input/TeX", "input/MathML", "input/AsciiMath", "output/SVG", "output/CommonHTML", "output/PreviewHTML"],
    ...
    AuthorInit: function () {
      ...
      //
      //  Adjust the PreviewHTML output jax
      //
      MathJax.Hub.Register.StartupHook("PreviewHTML Jax Config",function () {
        PHTMLSTYLES = MathJax.Ajax.StyleString(MathJax.OutputJax.PreviewHTML.config.styles);
      });
      //
...
function GetPHTML(result) {
  if (data.pcss) result.pcss = PHTMLSTYLES;
  if (!data.phtml && !data.phtmlNode) return;
  var jax = MathJax.Hub.getAllJax()[0]; if (!jax) return;
  var script = jax.SourceElement(), html = script.previousSibling;

  // add speech text if there isn't one
  if (data.speakText){
    var labelTarget = html.querySelector('.MJXp-math');
    for (child of labelTarget.childNodes) child.setAttribute("aria-hidden",true);
    if (!labelTarget.getAttribute("aria-label")){
    labelTarget.setAttribute("aria-label",result.speakText);
    }
  ...
  // remove automatically generated IDs
  var ids = html.querySelectorAll('[id^="MJXp-Span-"]');
  for (var i = 0; i < ids.length; i++){
    ids[i].removeAttribute("id");
  }
  // remove extreneous frame element
  ...
  if (data.phtml) result.phtml = html.outerHTML;
  if (data.phtmlNode) result.phtmlNode = html;
}
...
function StartQueue() {
  ...
  //
  var renderer = 'None';
  if (data.html || data.htmlNode || data.css) renderer = "CommonHTML";
  else if (data.svg || data.svgNode) renderer = "SVG"
  else if (data.phtml || data.phtmlNode || data.pcss) renderer = "PreviewHTML"
...
function RerenderPHTML(result) {
  if ((data.html || data.htmlNode || data.css) && (data.phtml || data.phtmlNode || data.pcss)) {
    timer = setTimeout(RestartMathJax,data.timeout);
    var queue = MathJax.Callback.Queue(), $$ = window.Array;
    return queue.Push(
      $$(SetRenderer,"PreviewHTML"),
...
function RerenderSVG(result) {
  if ((data.html || data.htmlNode || data.css || data.phtml || data.phtmlNode || data.pcss) && (data.svg || data.svgNode)) {
    timer = setTimeout(RestartMathJax,data.timeout);
    var queue = MathJax.Callback.Queue(), $$ = window.Array;
    return queue.Push(
      $$(SetRenderer,"SVG"),
