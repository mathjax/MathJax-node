...
var batikPath = dirname(dirname(require.resolve("./mj-single.js"))) + "/batik/";
...
function AddError(message,nopush) {
  if (displayErrors) console.log(message);
...
function GetSVG(result) {
  ...
  if (data.png) {
    ...
    exec("java -jar " + batikPath + "batik-rasterizer.jar -dpi "+data.dpi+" "+tmpfile+".svg",function (err,stdout,stderr) {
